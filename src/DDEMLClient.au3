...
; Author(s):       doudou
;===============================================================================
Func _DdeClientTransaction($wType, $hConv, $data = 0, $dwTimeout = 10000, $hszItem = 0, $wFmt = 0)
    ...
    If $XTYP_EXECUTE = $wType Then $wFmt = 0
    If $XTYP_EXECUTE = $wType Then $hszItem = 0

    DllStructSetData($stResult, 1, 0)
    ConsoleWrite("_DdeClientTransaction(" & VarGetType($data) & ", " & $cb & ")" & @CRLF)
